...
export default function SpreadsheetViewer() {
  ...
  const searchSuggestions = useMemo(() => {
     ...
     data.sheets.forEach(sheet => {
       if (selectedNamespace === 'all' || sheet.sheetId.toString() === selectedNamespace) {
  ...
  const namespaceOptions = useMemo(() => {
    ...
    if (data?.sheets) {
      data.sheets.forEach(sheet => options.push({ value: sheet.sheetId.toString(), label: sheet.title }))
    }
    return options
  }, [data])
  ...
  const stats = useMemo(() => {
    if (!data?.sheets) return { totalKeys: 0, completed: 0, missing: 0 }
    const targetSheets = selectedNamespace === "all" ? data.sheets : data.sheets.filter(s => s.sheetId.toString() === selectedNamespace)
    
    let total = 0, missing = 0
    targetSheets.forEach(sheet => {
      if (!sheet?.rows) return
      total += sheet.rows.length
      missing += sheet.rows.filter(row => 
        selectedLocales.length > 0 && selectedLocales.some(lang => !(row.data?.[lang] ?? "").trim())
      ).length
    })
    ...
    return { 
      ...
      percent: total > 0 ? Math.round(((total - missing) / total) * 100) : 100 
    }
  }, [data, selectedNamespace, selectedLocales])
    ...
    (fallbackLanguage: string | undefined, selectedLanguages: string[]) => {
      ...
      Object.entries(translations).forEach(([lang, content]) => {
        if (selectedLanguages.includes(lang)) {
          downloadJSON(`${lang.toLowerCase().trim()}.json`, content)
        }
      })
    }, [data]
  )
  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-hidden">
      ...
      <div className="flex-shrink-0 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/20 bg-white/40 backdrop-blur-md">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">
             <Link href="/profile" className="hover:text-blue-600 transition-colors">Dự án</Link>
        ...
        <div className="flex items-center gap-2">
           ...
           <Button
             ...
             icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>}
           >
             Xuất dữ liệu
           </Button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="hidden lg:flex w-72 flex-shrink-0 flex-col border-r border-white/20 bg-white/20 backdrop-blur-md p-6 space-y-8 overflow-y-auto">
           ...
           <div className="space-y-4">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Tiến độ tổng quan</label>
           ...
           <div className="space-y-4">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Danh mục nhanh</label>
             <div className="flex flex-col gap-1">
                {namespaceOptions.map(opt => (
                  ...
                  >
                     <span className={`w-1.5 h-1.5 rounded-full ${selectedNamespace === opt.value ? 'bg-white' : 'bg-slate-300'}`} />
                     {opt.label}
                  </button>
                ))}
             </div>
           ...
           <div className="pt-4 mt-auto">
              <div className="p-4 rounded-2xl bg-blue-50 border border-blue-100 text-blue-800 text-center">
                 <p className="text-[10px] font-black uppercase tracking-widest mb-1">Mẹo hay</p>
                 <p className="text-[10px] font-medium leading-relaxed opacity-80">Sử dụng phím ⌘/Ctrl + F để tìm kiếm nhanh từ khóa.</p>
              </div>
           </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
           ...
           {/* Keys Container */}
           <div className={`flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scrollbar transition-opacity duration-300 ${isSearching ? 'opacity-50' : 'opacity-100'}`}>
              ...
              <div className="lg:hidden grid grid-cols-2 sm:grid-cols-4 gap-3">
                 <div className="p-4 rounded-2xl bg-white border border-slate-200 text-center">
                    <div className="text-xs font-bold text-slate-400 uppercase mb-1">Dự án</div>
                    <div className="text-xl font-black text-slate-800">{stats.percent}%</div>
                 </div>
                 <div className="p-4 rounded-2xl bg-white border border-slate-200 text-center">
                    <div className="text-xs font-bold text-slate-400 uppercase mb-1">Hoàn thành</div>
                    <div className="text-xl font-black text-blue-600">{stats.completed}</div>
                 </div>
                 <div className="p-4 rounded-2xl bg-white border border-slate-200 text-center">
                    <div className="text-xs font-bold text-slate-400 uppercase mb-1">Cần dịch</div>
                    <div className="text-xl font-black text-amber-600">{stats.missing}</div>
                 </div>
                 <div className="p-4 rounded-2xl bg-white border border-slate-200 text-center">
                    <div className="text-xs font-bold text-slate-400 uppercase mb-1">Tổng cộng</div>
              ...
              {filtered?.sheets?.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 text-center space-y-6" style={{ animation: 'fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) both' }}>
                   ...
                   <div className="space-y-1">
                      <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">Không tìm thấy kết quả</h3>
                      <p className="text-slate-500 font-medium">Thử điều chỉnh bộ lọc hoặc từ khóa tìm kiếm của bạn.</p>
                   </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-6 pb-20">
                  {filtered?.sheets?.map((sheet, index) => sheet ? (
                    <div 
                      key={sheet.sheetId} 
