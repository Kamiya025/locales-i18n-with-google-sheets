...
export function SpreadsheetItemViewer(
  ...
  const isLazy =
    (sheet as any)._isLazyLoaded && (!sheet.rows || sheet.rows.length === 0)
  ...
  const mutationDeleteSheet = useDeleteSheet(
    (updatedData) => {
      ...
      customToast.success(
        t("detail.sheet.deleteSuccess").replace("{title}", sheet.title),
      )
    },
    (error) => {
  ...
  const { missingCount, progress } = useMemo(() => {
    if (selectedLocales.length === 0) return { missingCount: 0, progress: 100 }
    const missing = calculateMissingTranslations(sheet, selectedLocales)
    const rows = sheet.rows || []
    const completed = rows.length - missing
    const progressPercent =
      rows.length > 0 ? Math.round((completed / rows.length) * 100) : 100
  ...
  const handleToggle = async (open: boolean) => {
    if (open && isLazy && data && !isLoadingLazy) {
      ...
      try {
        const updatedSpreadsheet = await sheetApi.getSpecificSheet(
          data.id,
          sheet.title,
        )
