...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.spreadsheetId || body.sheetId === undefined) {
      return NextResponse.json(
        {
          success: false,
          error: "Missing required fields: spreadsheetId, sheetId",
        },
        { status: 400 }
      )
    }

    const { spreadsheetId, sheetId } = body
    ...
    try {
      ...
      if (session?.accessToken) {
        ...
        updatedSpreadsheet = await userService.deleteSheet(
          spreadsheetId,
          Number(sheetId)
        )
        authType = "user"
        ...
        updatedSpreadsheet = await googleSheetsService.deleteSheet(
          spreadsheetId,
          Number(sheetId)
        )
